...
- `api/` + `web/` — interfaz web (fases I0 a I3 de `PLAN_INTERFAZ.md`): FastAPI sirviendo `/api` y las tres páginas estáticas de `web/` (`index.html` con el timeline, los carriles de acción y las métricas; `reunion.html` con la reunión y su transcripción; `acciones.html` con el tablero de acciones). **Es lo único del repo que no es stdlib**; sus dependencias van en `requirements-api.txt`, aparte, para que el pipeline no dependa de ellas.
...
- **Tests**: `python -m unittest discover -s tests -v` (96). Solo stdlib, sin pytest, para que funcionen en el servidor offline sin instalar nada. Cubren la identidad estable, la conexión de solo lectura, WAL, el remapeo de rutas, la migración 1→2, las métricas y los carriles de I1, el cierre de la conexión desde otro hilo, la vista de reunión de I2 (arrastres vistos desde las dos reuniones, `.srt` y `.md` reconstruidos), el tablero de I3 (filtros, recuentos y órdenes) y —como red de seguridad— la idempotencia de los arrastres de la Fase 2.
