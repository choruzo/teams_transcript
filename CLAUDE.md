...
Herramienta local para grabar reuniones de Teams, transcribirlas con Whisper, resumirlas con un LLM local y consultarlas desde una web. No hay build system ni linter: es Python puro ejecutado directamente, con tests de `unittest` para `memoria.py` y la API.
...
- `api/` + `web/` — interfaz web (fases I0 a I4 de `PLAN_INTERFAZ.md`): FastAPI sirviendo `/api` y las cuatro páginas estáticas de `web/` (`index.html` con el timeline, los carriles de acción y las métricas; `reunion.html` con la reunión y su transcripción; `acciones.html` con el tablero de acciones; `buscar.html` con la búsqueda de texto completo). **Es lo único del repo que no es stdlib**; sus dependencias van en `requirements-api.txt`, aparte, para que el pipeline no dependa de ellas.
...
- **Tests**: `python -m unittest discover -s tests -v` (122). Solo stdlib, sin pytest, para que funcionen en el servidor offline sin instalar nada. Cubren la identidad estable, la conexión de solo lectura, WAL, el remapeo de rutas, la migración 1→2, las métricas y los carriles de I1, el cierre de la conexión desde otro hilo, la vista de reunión de I2 (arrastres vistos desde las dos reuniones, `.srt` y `.md` reconstruidos), el tablero de I3 (filtros, recuentos y órdenes), la búsqueda de I4 (traducción de la consulta, acentos, resaltado y sincronía del índice FTS al reprocesar) y —como red de seguridad— la idempotencia de los arrastres de la Fase 2.
